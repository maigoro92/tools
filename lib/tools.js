...
const koi_contract = "zo2y4uDh6J6tTQNo8ZaESLD8Ma7u9XX42tfukSmQ9vI";
...
class koi {
  ...
  */
  async loadWallet(walletFileLocation) {
    ...
    //       dataB = data;
    //       console.log(dataB[0].totalVoted);
    //     });
    //   }
    // });
 ...
 */
  ...
  */
  ...
  async vote(arg) {
    ...
    if (tx) {
      this.totalVoted += 1;
      await db.update({ totalVoted }, { totalVoted: this.totalVoted });
      return tx;
    }
    ...
    if (reciept) {
      if (reciept.status == 200) {
        this.totalVoted += 1;
        console.log(this.totalVoted);
        await db.update({}, { totalVoted: this.totalVoted });
        let data = reciept.data.receipt;
        await db.update({}, { $push: { reciept: data } });
